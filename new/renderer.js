...
function renderSidebar() {
  ...
  profiles.forEach((p) => {
    ...
    btn.title = `${p.name} — click để mở session; phải chuột để sửa`;
...
function openModal(profileToEdit = null) {
  ...
  modalTitle.innerText = profileToEdit ? 'Chỉnh sửa phiên' : 'Thêm phiên';
...
document.getElementById('modal-delete').onclick = () => {
  ...
  const action = confirm(`Xóa phiên [${editingProfile.name}]? Session/cookie của nó sẽ mất khi xóa view.`);
  if (action) {
...
document.getElementById('modal-save').onclick = () => {
  ...
  if (!name) {
    alert('Nhập tên phiên.');
    return;
  }

  const platform = appSelect.value || DEFAULT_PLATFORM;
  ...
  else switchProfile(editingProfile.id);
};
...
ipcRenderer.on('update-profile-avatar', (event, { id, avatarUrl }) => {
  ...
  if (p) {
    const isAutoAvatar =
      !p.avatar || p.avatar.includes('graph.facebook.com') || p.avatar.includes('scontent') || p.avatar.includes('fbcdn');
