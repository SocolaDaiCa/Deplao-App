...
for (const p of profiles) {
  if (!p.platform || p.platform === 'zalo' || p.platform === 'teams') p.platform = PLATFORM_ID;
  delete p.proxy;
}

if (profiles.length === 0) {
  profiles = [{ id: Date.now().toString(), name: 'Nick 1', partition: 'persist:nick_1', platform: PLATFORM_ID }];
  saveProfiles();
}
...
function openModal(profileToEdit = null) {
  ...
  modalTitle.innerText = profileToEdit ? 'Chỉnh sửa tài khoản' : 'Thêm tài khoản Messenger';
...
document.getElementById('modal-save').onclick = () => {
  ...
  if (editingProfile) {
    ...
    editingProfile.platform = PLATFORM_ID;
  } else {
